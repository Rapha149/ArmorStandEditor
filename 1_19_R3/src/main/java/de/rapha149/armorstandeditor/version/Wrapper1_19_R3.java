...
public class Wrapper1_19_R3 implements VersionWrapper {
    ...
    public String getCustomNameForEdit(ArmorStand armorStand) {
        ...
        return EDIT_SERIALIZER.serialize(GSON_SERIALIZER.deserialize(ChatSerializer.a(handle.ab())));
    }
    ...
    public Component getCustomNameForDisplay(ArmorStand armorStand) {
        ...
        return GSON_SERIALIZER.deserialize(ChatSerializer.a(handle.ab()));
    }
    ...
    public void resetArmorStandBodyPart(ArmorStand armorStand, BodyPart bodyPart) {
        EntityArmorStand handle = ((CraftArmorStand) armorStand).getHandle();
        switch (bodyPart) {
            case HEAD -> handle.a(defaultArmorStand.cb);
            case BODY -> handle.b(defaultArmorStand.cc);
            case LEFT_ARM -> handle.c(defaultArmorStand.cd);
            case RIGHT_ARM -> handle.d(defaultArmorStand.ce);
            case LEFT_LEG -> handle.e(defaultArmorStand.cf);
            case RIGHT_LEG -> handle.f(defaultArmorStand.cg);
        }
    }
    ...
    public void resetArmorStandBodyPart(ArmorStand armorStand, BodyPart bodyPart, Axis axis) {
        ...
        switch (bodyPart) {
            case HEAD:
                currentAngle = handle.cb;
                defaultAngle = defaultArmorStand.cb;
                break;
            case BODY:
    ...
    public ItemStack getArmorstandItem(ArmorStand armorStand, NamespacedKey privateKey) {
        ...
        if (handle.aa())
            nbt.a("CustomName", ChatSerializer.b(handle.ab()));
