...
public abstract class Page {
    ...
    ItemBuilder applyNameAndLoreWithoutKeys(ItemBuilder builder, String name, String lore, boolean status) {
        builder.name(SERIALIZER.deserialize(name).decoration(TextDecoration.ITALIC, false));
        if (!lore.isEmpty()) {
            builder.lore(Arrays.stream(lore.replace("%status%", getMessage("armorstands.status." + (status ? "on" : "off")))
